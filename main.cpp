...
void value_increment(void)
{
    ...
    m2m_temp->set_value( value1 );
    printf( "temp: %3.2f\nhum : %3.2f", value1, value2 );

    value_increment_mutex.unlock();
}

void servo2_update(const char* /*object_name*/)
{
	float value = (float) m2m_servo2->get_value_float();
    servo2 = value;
    printf("PUT Servo2 %f\n", value );
}
...
{
    printf("PUT update %s %d\n", object_name, (int)m2m_put_res->get_value_int());
}
...
int main(void)
{
    ...
    hum_temp.read_id(&id);
    printf("BMP180  humidity & barometer    = 0x%X\r\n", id);    
    ...
    // Temperator Sensor
    m2m_temp = M2MInterfaceFactory::create_resource(m2m_obj_list, 3303, 0, 5700, M2MResourceInstance::FLOAT, M2MBase::GET_ALLOWED);
    
    // Servo
    m2m_servo2 = M2MInterfaceFactory::create_resource(m2m_obj_list, 10346, 0, 5536, M2MResourceInstance::FLOAT, M2MBase::GET_PUT_ALLOWED);
    if (m2m_servo2->set_value(0.0f) != true) {
        printf("m2m_servo2->set_value() failed\n");
        return -1;
    }
    if (m2m_servo2->set_value_updated_function(servo2_update) != true) {
        printf("m2m_servo2->set_value_updated_function() failed\n");
        return -1;
    }
